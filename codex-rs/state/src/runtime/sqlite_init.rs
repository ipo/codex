...
pub(super) async fn open_sqlite(
    ...
    let schema_state = check_database(path, migrator, spec, telemetry_override)
        .await
        .map_err(|source| recovery::RuntimeDbInitError::new(spec.label, "check", path, source))?;
    ...
    pool_result.map_err(|source| {
        recovery::RuntimeDbInitError::new(spec.label, "open", path, source).into()
    })
}
...
async fn ensure_initialized(
    ...
    let _lock = lock_result
        .map_err(|source| recovery::RuntimeDbInitError::new(spec.label, "lock", path, source))?;
    ...
    let mut connection = connection_result.map_err(|source| {
        recovery::RuntimeDbInitError::new(spec.label, "bootstrap", path, source)
    })?;
    ...
    if plan.needs_migration {
        ...
        if let Err(source) = migrate_result {
            ...
            return Err(
                recovery::RuntimeDbInitError::new(spec.label, "migrate", path, source).into(),
            );
        }
    }
    connection.close().await.map_err(|source| {
        recovery::RuntimeDbInitError::new(spec.label, "bootstrap", path, source.into())
    })?;
        ...
        .map_err(|source| recovery::RuntimeDbInitError::new(spec.label, "check", path, source))?
    {
        ...
        SchemaState::NeedsInitialization(_) => Err(recovery::RuntimeDbInitError::new(
            spec.label,
            "initialize",
            path,
...
fn is_transient_lock_error(error: &anyhow::Error) -> bool {
    error.chain().any(|source| {
        ...
        if recovery::sqlite_error_detail_is_lock(database_error.message()) {
            return true;
        }
