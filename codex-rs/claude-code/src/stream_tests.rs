...
use crate::mock::block_start;
use crate::mock::block_stop;
use crate::mock::delta;
use crate::mock::event;
use crate::mock::finish;
use crate::mock::message_start as start;
use crate::mock::minimal_stream as minimal;
...
#[test]
fn incremental_decoder_handles_fragmented_frames_and_rejects_truncated_streams() {
    let fixture = minimal("end_turn".into(), json!({"output_tokens":2}));
    let mut decoder = IncrementalDecoder::new(|_| {});
    for chunk in fixture.as_bytes().chunks(7) {
        decoder.feed(chunk).unwrap();
    }
    assert_eq!(
        decoder.finish().unwrap().terminal_outcome,
        TerminalOutcome::Completed
    );

    let mut truncated = IncrementalDecoder::new(|_| {});
    truncated
        .feed(start(json!({"input_tokens":1,"output_tokens":0})).as_bytes())
        .unwrap();
    assert_eq!(
        truncated.finish(),
