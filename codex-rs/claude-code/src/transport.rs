...
pub enum NativeStreamError {
    ...
    #[error("native Claude request ended with terminal outcome {0:?}")]
    UnsuccessfulTerminal(TerminalOutcome),
}
...
impl<T: HttpTransport> ClaudeHttpAdapter<T> {
    ...
    pub async fn stream_request(
        ...
        tokio::spawn(async move {
            ...
            match decoder
                .finish()
                .map_err(NativeStreamError::from)
                .and_then(canonical_events)
            {
                Ok(events) => {
                    for event in events {
...
fn presentation_event(delta: PresentationDelta) -> ResponseEvent {
    match delta {
        PresentationDelta::Text { delta, .. } => ResponseEvent::OutputTextDelta(delta),
...
fn canonical_events(decoded: DecodedStream) -> Result<Vec<ResponseEvent>, NativeStreamError> {
    let end_turn = match decoded.terminal_outcome {
        TerminalOutcome::Completed | TerminalOutcome::ToolsReady => Some(true),
        TerminalOutcome::Continue => Some(false),
        outcome @ (TerminalOutcome::OutputExhausted | TerminalOutcome::Refusal) => {
            return Err(NativeStreamError::UnsuccessfulTerminal(outcome));
        }
    };
