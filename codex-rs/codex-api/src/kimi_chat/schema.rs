...
pub fn normalize_kimi_schema(schema: &Value) -> Result<Value, KimiSchemaError> {
    let root = schema.as_object().ok_or(KimiSchemaError::NotObject)?;
    let mut definitions = BTreeMap::new();
    for table in ["$defs", "definitions"]
        .into_iter()
        .filter_map(|key| root.get(key))
    {
        for (name, value) in table.as_object().ok_or(KimiSchemaError::NotObject)? {
            if definitions.insert(name.clone(), value.clone()).is_some() {
                return Err(KimiSchemaError::UnsupportedReference(format!(
...
fn visit(
    ...
    complete_type: bool,
) -> Result<(), KimiSchemaError> {
    let object = schema.as_object_mut().ok_or(KimiSchemaError::NotObject)?;
    if let Some(reference) = object.remove("$ref") {
        let reference = reference
            .as_str()
            .ok_or_else(|| KimiSchemaError::UnsupportedReference(reference.to_string()))?;
        let name = local_reference_name(reference)?;
        if stack.contains(&name) {
            return Err(KimiSchemaError::RecursiveReference(reference.to_string()));
        }
        let mut resolved = definitions
            .get(&name)
            .cloned()
            .ok_or_else(|| KimiSchemaError::UnresolvedReference(reference.to_string()))?;
        resolved
            .as_object_mut()
            .ok_or(KimiSchemaError::NotObject)?
    ...
    for key in MAP_SLOTS.split_ascii_whitespace() {
        ...
        {
            return Err(KimiSchemaError::MissingRequiredProperty(name.to_string()));
        }
    }
...
fn local_reference_name(reference: &str) -> Result<String, KimiSchemaError> {
    reference
        .strip_prefix("#/$defs/")
...
fn infer_value_type(values: &[Value]) -> Result<&'static str, KimiSchemaError> {
    ...
    if inferred.len() != 1 {
        return Err(KimiSchemaError::AmbiguousValueType);
    }
    inferred
        .pop_first()
        .ok_or(KimiSchemaError::AmbiguousValueType)
}
