    ...
    while let Ok(event) = rx.try_recv() {
        match event {
            ...
            AppEvent::OpenModelSelectionScopePrompt { model, effort } => {
                ...
                assert!(
                    render_bottom_popup(&chat, /*width*/ 80).contains("Apply model change"),
                    "expected model scope menu to open"
                );
            }
...
#[tokio::test]
async fn initial_recognized_command_with_args_matches_queued_composer_dispatch() {
...
async fn initial_goal_respects_feature_gating() {
    ...
    let rendered = events
        ...
        .collect::<Vec<_>>()
        .join("\n");
    insta::assert_snapshot!("initial_goal_feature_disabled", rendered);
    assert!(
        !events
            .iter()
...
async fn initial_unknown_slash_reports_the_existing_error() {
    ...
    let rendered = drain_insert_history(&mut rx)
        ...
        .collect::<Vec<_>>()
        .join("\n");
    insta::assert_snapshot!("initial_unknown_slash", rendered);
    assert_no_submit_op(&mut op_rx);
}
...
async fn initial_ordinary_message_keeps_the_complete_submission_payload() {
    ...
    assert_eq!(actual, next_submit_op(&mut op_rx));
}
