...
#[tokio::test]
async fn nonblocking_startup_command_immediately_drains_queued_input() -> Result<()> {
    let (mut app, mut app_event_rx, mut op_rx) = make_test_app_with_channels().await;
    replace_test_chat_widget_with_initial_message(
        &mut app,
...
async fn set_thread_goal_draft_materializes_long_objective_and_confirms_before_paste() -> Result<()>
{
    ...
    app.set_thread_goal_draft(
        ...
        thread_id,
        crate::goal_files::GoalDraft {
            objective: "Inspect this image".to_string(),
            local_images: vec![crate::bottom_pane::LocalImageAttachment {
    ...
    let objective = app_server
        ...
        .await?
        .goal
        .expect("unpositioned image goal should be set")
        .objective;
    let copied_image = objective
        .strip_prefix("Inspect this image\n\nReferenced image files:\n- [Image #1]: ")
        .expect("unpositioned image should be appended to the objective");
    assert_eq!(std::fs::read(copied_image)?, b"png bytes");
    app_server.shutdown().await?;
    Ok(())
}
