...
async fn missing_native_route_fails_during_thread_start_without_sampling() -> Result<()> {
    ...
    let result = test_codex()
        .with_model_info_override("gpt-5.5", |model| {
            model.inference = Some(ModelInferenceConfig::Anthropic {
    ...
    assert_eq!(
        error.to_string(),
        "model `gpt-5.5` (anthropic) requires provider wire route `claude_code`; configure `model_providers.<provider>.wire_routes.claude_code` with `wire_api = \"anthropic_messages\"` and `dialect = \"claude_code\"`"
    );
    assert!(
        server
            .received_requests()
...
async fn family_incompatible_matching_route_fails_before_sampling() -> Result<()> {
    ...
    assert_eq!(
        error.to_string(),
        "model `gpt-5.5` declares an incompatible inference contract for family `anthropic`: `wire_api = \"responses\"` with `dialect = \"open_ai\"`; supported: `wire_api = \"anthropic_messages\"` with `dialect = \"claude_code\"`"
    );
    assert!(
        server
            .received_requests()
