...
async fn captured_responses_body(server: &MockServer, home: Arc<TempDir>) -> Result<Value> {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn openai_responses_keep_complete_body() -> Result<()> {
    skip_if_no_network!(Ok(()));
    let openai_server = responses::start_mock_server().await;
    let openai = captured_responses_body(&openai_server, Arc::new(TempDir::new()?)).await?;
    insta::assert_snapshot!(
        "kimi_conformance_established_openai_body",
