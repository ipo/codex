...
async fn server_overload_retries_use_a_separate_budget_on_the_same_turn() -> anyhow::Result<()> {
    ...
    let responses = mount_response_sequence(
        ...
        vec![
            ...
            .insert_header("x-codex-turn-state", "capacity-retry-state"),
    ...
    let request_input = requests[0].body_json()["input"].clone();
    assert!(
        requests.iter().all(
            |request| request.body_json()["client_metadata"]["turn_id"] == turn_id
                && request.body_json()["input"] == request_input
        )
    );
    ...
    assert!(
        requests[1..]
            .iter()
            .all(|request| request.header("x-codex-turn-state")
                == Some("capacity-retry-state".to_string()))
    );
...
#[tokio::test]
async fn server_overload_retry_delay_is_interruptible() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    let server = MockServer::start().await;
    let responses = mount_response_sequence(
        &server,
        vec![ResponseTemplate::new(503).set_body_json(json!({
            "error": {
                "code": "slow_down",
                "message": "selected model is at capacity"
            }
    ...
    test.codex
        .submit(Op::UserInput {
            items: vec![UserInput::Text {
                text: "interrupt capacity retry".into(),
                text_elements: Vec::new(),
            }],
            final_output_json_schema: None,
            responsesapi_client_metadata: None,
...
#[tokio::test]
async fn guardian_sampling_does_not_enter_core_capacity_retry_loop() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    let server = MockServer::start().await;
