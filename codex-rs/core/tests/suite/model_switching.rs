...
async fn marked_model_omits_empty_assistant_message_before_tool_call() -> Result<()> {
    ...
    assert!(second_request.message_input_texts("assistant").is_empty());
    assert_eq!(second_request.inputs_of_type("function_call").len(), 1);
    assert_eq!(
        second_request.inputs_of_type("function_call_output").len(),
        1
    );
    Ok(())
}
