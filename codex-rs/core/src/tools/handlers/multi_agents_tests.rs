...
async fn multi_agent_v2_full_fork_inherits_exact_model_and_effort_and_bypasses_defaults() {
    ...
    let mut config = (*turn.config).clone();
    config.agent_default_subagent_model = Some("openai/gpt-test".to_string());
...
async fn multi_agent_v2_bounded_and_clean_forks_enforce_history_groups() {
    ...
    let openai_id = session
        .services
        .agent_control
        .resolve_agent_reference(session.thread_id, &turn.session_source, "openai_plaintext")
        .await
        .expect("OpenAI child should resolve");
    ...
    let delivered_before = manager
        .captured_ops()
        .iter()
        .filter(|(id, op)| *id == openai_id && matches!(op, Op::InterAgentCommunication { .. }))
        .count();
        ...
        let result = if trigger_turn {
            FollowupTaskHandlerV2
                .handle(invocation(
                    ...
                    tool_name,
                    function_payload(
                        json!({"target": "openai_plaintext", "message": "ciphertext"}),
                    ),
                ))
                .await
        } else {
    ...
    assert_eq!(
        manager
            ...
            .iter()
            .filter(|(id, op)| {
                *id == openai_id && matches!(op, Op::InterAgentCommunication { .. })
            })
            .count(),
        delivered_before
    );
