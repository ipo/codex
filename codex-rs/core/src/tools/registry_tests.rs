...
fn flattened_aliases_reject_plain_name_collisions() {
    ...
    let plain = Arc::new(TestHandler {
        tool_name: codex_tools::ToolName::plain("mcp__calendar__create_event"),
    }) as Arc<dyn CoreToolRuntime>;

    ToolRegistry::from_tools_with_namespace_tool_spec_mode(
        [namespaced, plain],
        codex_tools::NamespaceToolSpecMode::Flatten,
    );
}
