...
async fn multi_agent_v2_namespace_follows_resolved_model_inference_contract() {
    ...
    kimi.assert_visible_lacks(&[MULTI_AGENT_V2_NAMESPACE]);
    kimi.assert_visible_contains(&collaboration_tools);
    for tool_name in collaboration_tools {
        let ToolSpec::Function(kimi_function) = kimi.visible_spec(tool_name) else {
        ...
        assert_eq!(kimi_function, responses_function);
        kimi.assert_registered_contains(&[tool_name]);
        assert!(
            !kimi
                .registered_names
                .contains(&ToolName::namespaced(MULTI_AGENT_V2_NAMESPACE, tool_name).to_string()),
            "Kimi should register `{tool_name}` without a namespace"
        );
    }
}
