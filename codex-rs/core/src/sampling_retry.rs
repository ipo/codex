//! Provider-aware retry decisions for native model sampling requests.
...
pub(crate) async fn handle_native_sampling_retry(
    ...
    *retries += 1;
    ...
    codex_client::record_retry!(retry_count, delay, RetryOperation::Sampling);
...
fn classify_native_error_with_scheduler(
    ...
    match error {
        ...
        NativeStreamError::Request(ApiError::RateLimit(message)) => retryable(message, None),
        ...
        NativeStreamError::UnsuccessfulTerminal(TerminalOutcome::OutputExhausted) => {
            ApiError::InvalidRequest {
                message: "model output limit reached before the turn completed".to_string(),
            }
        }
        NativeStreamError::UnsuccessfulTerminal(TerminalOutcome::Refusal) => {
            ApiError::InvalidRequest {
                message: "model refused to complete the turn for safety reasons".to_string(),
            }
        }
...
fn classify_transport_error(error: TransportError, now: DateTime<Utc>) -> ApiError {
    match error {
        ...
        TransportError::Connection(_) => retryable(error.to_string(), None),
